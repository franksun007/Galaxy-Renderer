...
public:
  ...
  AffineDrawer(const float scale, Sphere* sphere, Vec3D color): scale(scale), sphere(sphere), color(color), rotate(60.0) {};
  ...
  void Update() {
    ...
    Vec3D new_velocity;
    new_velocity.x = sphere->velocity.x * std::cos(angle) + sphere->velocity.y * -std::sin(angle);
    new_velocity.y = sphere->velocity.x * std::sin(angle) + sphere->velocity.y * std::cos(angle);
...
void mod(Context *context) {
  ...
  center_drawer = new(center_drawer) opengl::SphereDrawer(1.0, the_center, green);
  opengl::AffineDrawer* orbitor_drawer = context->Borrow<opengl::AffineDrawer>();
...
int32_t main(int32_t argc, char **argv) {
  ...
  Context c(2000, 2000, 35000.0, "Some Caption");
