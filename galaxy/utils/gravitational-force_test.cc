...
TEST(GravitationalForceTest, test_scaled_gforce) {
  ...
  ASSERT_NEAR(ScaledGravitationalForce(mass_a, mass_b, distance, scale),
              static_cast<float>(reference * scale), 1e-7);
}
...
TEST(GravitationalForceTest, test_relative_gforce) {
  ...
  ASSERT_FLOAT_EQ(RelativeGravitationalForce(mass_a, mass_b, distance),
